...
OQS_KEM *OQS_KEM_hqc_256_new(void) {
	...
	kem->length_encaps_seed = OQS_KEM_hqc_256_length_encaps_seed;
...
OQS_API OQS_STATUS OQS_KEM_hqc_256_encaps_derand(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *seed) {
	(void)ciphertext;
	(void)shared_secret;
	(void)public_key;
	(void)seed;
